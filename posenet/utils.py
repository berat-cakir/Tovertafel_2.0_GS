...
# Chinmay - Modified to make input image as gray-scale
def process_input(source_img, scale_factor=1.0, output_stride=16):
    ...
    # Convert source image into gray -- just to display
    source_img_gray = cv2.cvtColor(source_img, cv2.COLOR_BGR2GRAY)

    input_img = cv2.resize(source_img, (target_width, target_height), interpolation=cv2.INTER_LINEAR)
    # Convert input image into gray
    input_img_gray = cv2.cvtColor(input_img, cv2.COLOR_BGR2GRAY)
    # Stack up the same grays to get 3 channels
    input_img = np.dstack([input_img_gray, input_img_gray, input_img_gray])
    # Nomalize
    input_img = input_img * (2.0 / 255.0) - 1.0
    # Reshape into a form accepted by Posenet
    input_img = input_img.reshape(1, target_height, target_width, 3)

    return input_img, source_img_gray, scale
...
def draw_skel_and_kp(
    ...
    for ii, score in enumerate(instance_scores):
        ...
        #print('instance_score', score)
        #print('keypoint_coords', keypoint_coords[ii,:,:])
        #print('keypoint_scores', keypoint_scores[ii,:])
